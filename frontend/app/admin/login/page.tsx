...
function PasswordStep({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-xs text-zinc-500 mb-1.5" htmlFor="username">Usuario</label>
      ...
      <div>
        <label className="block text-xs text-zinc-500 mb-1.5" htmlFor="password">Contraseña</label>
...
function OtpStep({ ephemeralToken, onSuccess, onBack }: {
  ephemeralToken: string; onSuccess: () => void; onBack: () => void;
}) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="text-center mb-2">
        <p className="text-zinc-500 text-sm">Ingresa el código de tu app autenticadora.</p>
      </div>
      <div>
        <label className="block text-xs text-zinc-500 mb-1.5" htmlFor="otp">Código de verificación</label>
        <input ref={inputRef} id="otp" type="text" inputMode="numeric" pattern="[0-9]*"
          maxLength={6} autoComplete="one-time-code" value={code}
...
function LoginForm() {
  ...
  useEffect(() => {
    authMe().then((u) => { if (u) router.replace("/admin/dashboard"); });
  }, [router]);
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm">
          {step === "credentials" ? (
            <PasswordStep
              ...
              onRequiresOtp={(token) => { setEphemeralToken(token); setStep("otp"); }}
            />
          ) : (
            <OtpStep
              ephemeralToken={ephemeralToken}
              onSuccess={handleSuccess}
              onBack={() => { setEphemeralToken(""); setStep("credentials"); }}
            />
          )}
        </div>
      </div>
...
function StepDot({ label, active, done }: { label: string; active?: boolean; done?: boolean }) {
  return (
    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${
      done || active ? "bg-red-500 text-white" : "bg-zinc-200 text-zinc-500"
    }`}>
      {done ? "✓" : label}
    </div>
  );
...
const BTN_CLS =
  "w-full bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white font-medium text-sm py-2.5 rounded-lg transition-colors";
