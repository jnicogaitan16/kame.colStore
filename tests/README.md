...
En **`.github/workflows/e2e.yml`** el job **`Playwright E2E`** sigue usando mock + Next local (sin Wompi real). En cada **push a `main`** (p. ej. tras merge), si configurás el secret, corre después **`Playwright Wompi Nequi (sandbox)`** (`needs: e2e`). En PRs no corre (solo el E2E con mock).
...
Los PR desde **forks** no reciben secrets; en **push a `main`** el secret del repo sí aplica.
