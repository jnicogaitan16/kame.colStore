...
> Auditoría inicial: 2026-04-09. Actualizado: 2026-04-11 — **E2E Wompi sandbox:** solo **`payments-nequi-sandbox.spec.ts`** + `payments-sandbox-helpers.ts` (flujo Nequi aprobado/declinado contra widget y backend reales); no hay specs sandbox para tarjeta/PSE/Daviplata/QR/Puntos (datos de prueba siguen en `wompi-sandbox.ts`). CI estándar: mock + stub en `checkout.spec.ts`. Job opcional en `e2e.yml` (push `main`/`feat/*`, PR `feat/*`→`main`, o manual; secret `E2E_SANDBOX_BASE_URL`): ver §6. **Django:** `GET /api/health/` mínimo para probes/túneles. **Pyright:** `pyrightconfig.json`. El **README raíz** no documenta el detalle de pagos E2E; este archivo y `tests/README.md` sí.
